    SELECT
    DOH.CUSTOMER_NUMBER||'-'||DOH.CUSTOMER_NAME CUSTOMER,
--    DOH.CUSTOMER_NUMBER,
--    DOH.CUSTOMER_NAME,
    OOH.ORDER_NUMBER,
...
--    TO_CHAR(MODT.VAT_CHALLAN_DATE) VAT_CHALLAN_DATE,
    ...
    (SELECT --WSHD.RELEASED_STATUS_NAME
     DECODE (WSHD.RELEASED_STATUS, 'B', 'Back Ordered', ' '
--                                        WSHD.RELEASED_STATUS_NAME
                                        ) "STATUS" 
    FROM APPS.WSH_DELIVERABLES_V WSHD WHERE 1=1
    AND OOL.HEADER_ID = WSHD.SOURCE_HEADER_ID AND OOL.LINE_ID = WSHD.SOURCE_LINE_ID AND WSHD.RELEASED_STATUS='B'
    ) DELIVERY_STATUS
