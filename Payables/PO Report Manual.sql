SELECT  :T_C3,:T_C4,:T_C5,:T_C6,:T_C7,:T_C8,:T_C9,:T_C10,:T_C11,:T_C12,pha.rate,TO_CHAR(pha.rate_date,'DD-MON-RRRR') rate_date,:P_MOF_PO,pdd.CODE_COMBINATION_ID,code_test.DESCRIPTION,
     pha.PO_HEADER_ID,pha.COMMENTS,pla.PO_line_ID,:p_org_id,PLA.LINE_NUM,PLA.NOTE_TO_VENDOR,
      NVL(PHA.AUTHORIZATION_STATUS,'INCOMPLETE') PO_STATUS,
       ...
       pll.po_release_id, pra.release_num, :p_del_date,TO_CHAR (pha.creation_date, 'DD-MON-RR') po_cr_dt,
      (select  NVL(flv.TAG ,'Purchase Order' ) from apps.PO_DISTRIBUTIONS_ALL pda, apps.gl_code_combinations_kfv gck , apps.fnd_lookup_values flv
     where pda.CODE_COMBINATION_ID=gck.CODE_COMBINATION_ID and gck.SEGMENT1=flv.LOOKUP_CODE and flv.LOOKUP_type='XXAKG_COMPANY_CODES' and rownum=1 and PO_HEADER_ID=:P_PO_NO)PO_TITLE,
      ...
      -- XXAKG_COM_PKG.GET_UNIT_NAME(:p_org_id) Org_header_name,
       --xxakg_com_pkg.get_hr_operating_unit(:p_org_id) Org_header_name,
        ( select distinct  flv.DESCRIPTION  from apps.PO_DISTRIBUTIONS_ALL pda, apps.gl_code_combinations_kfv gck , apps.fnd_lookup_values flv
       where pda.CODE_COMBINATION_ID=gck.CODE_COMBINATION_ID and gck.SEGMENT1=flv.LOOKUP_CODE and   flv.LOOKUP_type='XXAKG_COMPANY_CODES'   and pda.CODE_COMBINATION_ID  =pdd.CODE_COMBINATION_ID  group by  flv.DESCRIPTION) Org_header_name,
              ( select distinct  flv.attribute7  from apps.PO_DISTRIBUTIONS_ALL pda, apps.gl_code_combinations_kfv gck , apps.fnd_lookup_values flv
       where pda.CODE_COMBINATION_ID=gck.CODE_COMBINATION_ID and gck.SEGMENT1=flv.LOOKUP_CODE and   flv.LOOKUP_type='XXAKG_COMPANY_CODES'   and pda.CODE_COMBINATION_ID  =pdd.CODE_COMBINATION_ID  group by  flv.attribute7 ) PO_TITLE,   
              ( select distinct  flv.attribute8  from apps.PO_DISTRIBUTIONS_ALL pda, apps.gl_code_combinations_kfv gck , apps.fnd_lookup_values flv
       where pda.CODE_COMBINATION_ID=gck.CODE_COMBINATION_ID and gck.SEGMENT1=flv.LOOKUP_CODE and   flv.LOOKUP_type='XXAKG_COMPANY_CODES'   and pda.CODE_COMBINATION_ID  =pdd.CODE_COMBINATION_ID  group by  flv.attribute8 ) org_header_address, 
       ...
       pll.quantity po_qty,
      '('||pha.currency_code||')' Currency_code, pla.unit_price,pll.quantity*pla.unit_price amount, pla.attribute2 brand,
       ...
       pda.req_header_reference_num pr_no,ood.ORGANIZATION_CODE destinition,ood.ORGANIZATION_NAME dest_NAME,
   DECODE (pdD.REQ_DISTRIBUTION_ID,
              NULL,apps.XXAKG_P2P_PKG.XXAKG_FND_REQUISITION_INFO(PLL.ATTRIBUTE1,:P_ORG_ID,PLL.ATTRIBUTE2,'RNUM'),
        PRD.REQUISITION_NUM) requisition_no,
       :p_note, :p_release, :p_po_no,
       /*PAYMENT*/
  ...
  FROM apps.po_headers_all pha,
       ...
       apps.po_releases_all pra,APPS.PO_REQ_DISTRIBUTIONS_INQ_V PRD,
                  apps.PO_DISTRIBUTIONS_ALL pdd,
                          ( select distinct  flv.DESCRIPTION, pdaa.po_header_id  ,pdaa.CODE_COMBINATION_ID   from apps.PO_DISTRIBUTIONS_ALL pdaa, apps.gl_code_combinations_kfv gck , apps.fnd_lookup_values flv
       where pdaa.CODE_COMBINATION_ID=gck.CODE_COMBINATION_ID and gck.SEGMENT1=flv.LOOKUP_CODE and   
       flv.LOOKUP_type='XXAKG_COMPANY_CODES'     and    pdaa.po_header_id=:P_PO_NO ) code_test,
 ...
 WHERE pha.po_header_id = pla.po_header_id
   ...
   and pdd.GL_CANCELLED_DATE is null
   ...
   AND pla.unit_meas_lookup_code=MUOM.UNIT_OF_MEASURE(+)
                          and  pha.po_header_id = pdd.po_header_id
                          AND pla.po_line_id = pdd.po_line_id
                          and pha.po_header_id=code_test.po_header_id
                          and code_test.CODE_COMBINATION_ID=pdd.CODE_COMBINATION_ID
   ...
   AND NVL(PRA.CANCEL_FLAG,'N')='N'
    --AND NVL (PRA.closed_code, 'OPEN') not in ('CLOSED','FINALLY CLOSED')
  ...
  -- AND NVL(PRA.AUTHORIZATION_STATUS,'INCOMPLETE')=NVL(:P_REL_STUS,NVL(PHA.AUTHORIZATION_STATUS,'INCOMPLETE'))
   UNION ALL
  SELECT :T_C3,:T_C4,:T_C5,:T_C6,:T_C7,:T_C8,:T_C9,:T_C10,:T_C11,:T_C12,pha.rate,TO_CHAR(pha.rate_date,'DD-MON-RRRR') rate_date,:P_MOF_PO,0,'test',
      pha.PO_HEADER_ID,pha.COMMENTS,pla.PO_line_ID,:p_org_id,PLA.LINE_NUM,PLA.NOTE_TO_VENDOR,
      NVL(PHA.AUTHORIZATION_STATUS,'INCOMPLETE') PO_STATUS,
       TO_CHAR(NULL) RELEASE_STATUS,
       to_number(null) po_release_id, to_number(null) release_num, :p_del_date,
      ...
      'Work Order' PO_TITLE,
       ...
       'Subject: Work Order' PO_TITLE_SUB,
        -- XXAKG_COM_PKG.GET_UNIT_NAME(:p_org_id) Org_header_name,
       --xxakg_com_pkg.get_hr_operating_unit(:p_org_id) Org_header_name,
          ( select  flv.DESCRIPTION  from apps.PO_DISTRIBUTIONS_ALL pda, apps.gl_code_combinations_kfv gck , apps.fnd_lookup_values flv
       where pda.CODE_COMBINATION_ID=gck.CODE_COMBINATION_ID and gck.SEGMENT1=flv.LOOKUP_CODE and flv.LOOKUP_type='XXAKG_COMPANY_CODES' and rownum=1 and PO_HEADER_ID=:P_PO_NO) Org_header_name, 
              ( select distinct  flv.attribute7  from apps.PO_DISTRIBUTIONS_ALL pda, apps.gl_code_combinations_kfv gck , apps.fnd_lookup_values flv
       where pda.CODE_COMBINATION_ID=gck.CODE_COMBINATION_ID and gck.SEGMENT1=flv.LOOKUP_CODE and   flv.LOOKUP_type='XXAKG_PO_PRINT_COMPANY'  group by  flv.attribute7 ) PO_TITLE,  
        (select  flv.ATTRIBUTE12  from apps.PO_DISTRIBUTIONS_ALL pda, apps.gl_code_combinations_kfv gck , apps.fnd_lookup_values flv
      where pda.CODE_COMBINATION_ID=gck.CODE_COMBINATION_ID and gck.SEGMENT1=flv.LOOKUP_CODE and flv.LOOKUP_type='XXAKG_COMPANY_CODES' and rownum=1 and PO_HEADER_ID=:P_PO_NO) org_header_address,
       pha.FREIGHT_TERMS_LOOKUP_CODE Mode_transport,
       TO_CHAR (null) rel_dt,
       pha.segment1 po_num, DECODE(org.NAME,'AKM - Project','Abul Khair Steel',org.NAME) org_name,
       pov.vendor_name || ' ' || '(' || pov.SEGMENT1|| ')' supplier_name,
       ...
       --decode(pll.quantity,null,pla.QUANTITY,pll.quantity) po_qty,
       to_number(0) po_qty,
      '('||pha.currency_code||')' Currency_code, pla.unit_price, to_number(0)*pla.unit_price amount, pla.attribute2 brand,
       nvl(pha.revision_num,0) revision_num, pla.attribute3 origin,
