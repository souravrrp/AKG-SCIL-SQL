DROP MATERIALIZED VIEW APPS.XXAKG_GL_DTL_DMV_2041;
CREATE MATERIALIZED VIEW APPS.XXAKG_GL_DTL_DMV_2041 
...
WITH PRIMARY KEY
AS 
/* Formatted on 3/1/2015 12:34:41 PM (QP5 v5.136.908.31019) */
...
SELECT 222 APPLICATION_ID,
 ...
 WHERE     CUST.CUSTOMER_ID = CT.BILL_TO_CUSTOMER_ID
       ...
       AND TRUNC (XAH.ACCOUNTING_DATE) BETWEEN (SELECT TRUNC (
                                                          MIN (START_DATE))
                                                          Period_Start_Date
                                                  FROM gmf.gmf_period_statuses
                                                 WHERE legal_entity_id =
...
SELECT 260 APPLICATION_ID,
 ...
 WHERE     XTE.APPLICATION_ID = 260
       ...
       AND CC.SEGMENT3 = FLEX.FLEX_VALUE_MEANING
       AND XEH.LEDGER_ID = 2041
       AND TRUNC (XEH.ACCOUNTING_DATE) BETWEEN (SELECT TRUNC (
                                                          MIN (START_DATE))
                                                          Period_Start_Date
                                                  FROM gmf.gmf_period_statuses
                                                 WHERE legal_entity_id =
