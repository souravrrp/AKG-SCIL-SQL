/* Formatted on 5/20/2014 6:43:44 PM (QP5 v5.136.908.31019) */
  SELECT OPERATING_UNIT,
         ...
         ITEM_TYPE,
         ITEM_SOURCE,
         CASE WHEN TYP = 'OPN' THEN SUM(  OPENING_QTY+ ITR_RCV+ PO_RECEVING+ PO_CORRECTION_RCV+ WIP_RETUN+ WIP_COMPLETION+ WIP_BY_PRODUCT_RCV+ PRA_RCV+ RMA_RCV+ DORG_RCV+ MISC_RCV+ ITR_ISSUE+ MO_ISSUE+ NRG_ISSUE+ RTV_ISSUE+ PO_CORRCTION_ISSUE+ MI2C_ISSUE+ WIP_COMPLETION_RE_ISSUE+ WIP_ISSUE+ WIPBR_ISSUE+ PRA_ISSUE+ DORG_ISSUE+ MISC_ISSUE+ SLS_ISSUE) ELSE 0 END P_OPN_QTY,
         CASE
            WHEN TYP = 'OPN'
            THEN
               SUM(  S_OPENING_QTY
    ...
    FROM (SELECT TYP,
                 ...
                 S_SLS_ISSUE
                   apps.fnc_get_item_cost (
                      organization_id,
                      inventory_item_id,
                      TO_CHAR (trunc(:p_date_from),'MON-RR')) current_cost,
                   apps.fnc_get_item_cost (
                      organization_id,
                      inventory_item_id,
                      DECODE (TO_CHAR (trunc(:p_date_from),'DD'),
                         '01',TO_CHAR (trunc(:p_date_from)- 1,'MON-RR'),
                         TO_CHAR (trunc(:p_date_from),'MON-RR')))
                      OPN_COST
            FROM (WITH ON_HAND
                       ...
                       RCV
                          AS (  SELECT organization_id,
                                       ...
                                       SUM(DECODE (
                                              ...
                                              12,
                                              DECODE (
                                                 SIGN(mmt.secondary_transaction_quantity),
                                                 1,
                  ...
                  SELECT 'OPN' TYP,
                   ...
                   WHERE            --oh.organization_id (+)=r.organization_id
                         ...
                         AND msi.concatenated_segments = 'DRCT.FLSH.0001'
