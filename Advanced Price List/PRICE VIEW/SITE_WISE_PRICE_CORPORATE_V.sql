...
SELECT --PRODUCT_ATTR_VALUE "Item Code-Name", 
...
,substr(DESC_ATTRIB_VALUE_FROM,1,instr(DESC_ATTRIB_VALUE_FROM,'|',1,1)-10+instr(DESC_ATTRIB_VALUE_FROM,'|',1,1)) CUSTOMER_NAME_SITE
...
AND qualifier_attr_type IN ('Ship To') 
AND DECODE (
qualifier_attr_type,
'Ship To', SUBSTR (desc_attrib_value_from,1,INSTR (desc_attrib_value_from,'-')- 2)) IN ( SELECT ACSV.CUSTOMER_NUMBER FROM APPS.XXAKG_AR_CUSTOMER_SITE_V ACSV, APPS.XXAKG_DISTRIBUTOR_BLOCK_M DBM WHERE  ACSV.CUSTOMER_NUMBER=DBM.CUSTOMER_NUMBER AND DBM.REGION_NAME IN ('Corporate North','Corporate South','Institutional','MES')) 
) x
WHERE a.mod_line_no=x.mod_line_no
and a.group_no=x.group_no
and END_DATE is null
--AND substr(DESC_ATTRIB_VALUE_FROM,1,instr(DESC_ATTRIB_VALUE_FROM,'-',1,1)-1) LIKE '%186788%'
AND OPERATOR!='NOT ='
AND modifier_name IN ('SCIL Corporate Fixed Rate For OPC Cement'
                                        ,'SCIL Corporate Fixed Rate For Special Cement'
                                        ,'SCIL Corporate Fixed Rate For OPC Bulk Cement'
