...
FINAL_DESTINATION, NULL DELIVERY_LOCATION, HIRE_RATE,chart_rate,VENDOR_SITE_CODE,
(select invoice_amount from APPS.ap_invoices_all where invoice_num=MOV_ORDER_NO and org_id=85) ap_inv_amt
...
DECODE(DOL.ORDERED_ITEM_ID, 24408, (DOL.LINE_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION),206570, (DOL.LINE_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION),24402, (DOL.LINE_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION),992039, (DOL.LINE_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION),DOL.LINE_QUANTITY) DO_QUANTITY, DOL.LINE_QUANTITY ORIGINAL_QUANTITY,
(CASE
 ...
 MVH.HIRE_RATE_AP HIRE_RATE,ASSA.VENDOR_SITE_CODE,
 (select payable_rate from apps.xxakg_transport_hirerate 
 where org_id=mvh.org_id
 and from_location=mvh.warehouse_org_id
and to_location=mvh.final_destination
and transport_mode=mvh.transport_mode
and mvh.MOV_ORDER_DATE between start_date_active and nvl(end_date_active,mvh.MOV_ORDER_DATE)
) chart_rate
,mvh.payment_type,mvh.demurrage
...
AND      DOL.ORDERED_ITEM_ID NOT IN (24397, 24398,856989,856991)   -->> EBAG.PBAG.0001 (24397), EBAG.SBAG.0001 (24398), EBAG.PBAG.0002(856989),EBAG.SBAG.0002(856991) 1466
...
AND    API.VENDOR_ID=ASSA.VENDOR_ID(+)
AND    API.VENDOR_SITE_ID=ASSA.VENDOR_SITE_ID(+)
...
DECODE (MVH.WAREHOUSE_ORG_ID, 101, MVH.WAREHOUSE_ORG_CODE, APPS.XXAKG_COM_PKG.GET_ORGANIZATION_NAME (MVH.WAREHOUSE_ORG_ID)) WAREHOUSE,  
DECODE(SOL.INVENTORY_ITEM_ID, 24408, (SOL.SHIPPED_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION),206570, (SOL.SHIPPED_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION),24402, (SOL.SHIPPED_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION),992039, (SOL.SHIPPED_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION), SOL.SHIPPED_QUANTITY) DO_QUANTITY, SOL.SHIPPED_QUANTITY ORIGINAL_QUANTITY,
...
MVH.HIRE_RATE_AP HIRE_RATE,NULL VENDOR_SITE_CODE,
(select payable_rate from apps.xxakg_transport_hirerate 
 where org_id=mvh.org_id
 and from_location=mvh.warehouse_org_id
and to_location=mvh.final_destination
...
--AND     MVH.TRANSPORT_MODE NOT IN ('Ex factory truck', 'Barge Ex factory') 
AND      SOL.INVENTORY_ITEM_ID NOT IN  (24397, 24398,856989,856991)   -->> EBAG.PBAG.0001 (24397), EBAG.SBAG.0001 (24398), EBAG.PBAG.0002(856989),EBAG.SBAG.0002(856991)
AND     DOH.CUSTOMER_NUMBER = CUST.CUSTOMER_NUMBER
AND     CUST.SITE_USE_CODE = 'BILL_TO'
AND     CUST.PRIMARY_FLAG = 'Y'
AND     DOH.CUSTOMER_NUMBER = R.CUSTOMER_NUMBER(+)
...
MOV_ORDER_STATUS, WAREHOUSE, BILL_STATUS, PAY_DATE, TRANSPORT_MODE,  FINAL_DESTINATION, HIRE_RATE,VEHICLE_NO,chart_rate,VENDOR_SITE_CODE
,payment_type,demurrage
...
DECODE(APPS.XXAKG_AP_PKG.get_invoice_status(API.INVOICE_ID), 'Never Validated', 'Invoiced', APPS.XXAKG_AP_PKG.get_invoice_status(API.INVOICE_ID))  MOV_ORDER_STATUS,  
DECODE (MVH.WAREHOUSE_ORG_ID, 101, MVH.WAREHOUSE_ORG_CODE, APPS.XXAKG_COM_PKG.GET_ORGANIZATION_NAME (MVH.WAREHOUSE_ORG_ID)) WAREHOUSE,  
DECODE(DOL.ORDERED_ITEM_ID, 24408, (DOL.LINE_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION),206570, (DOL.LINE_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION),DOL.LINE_QUANTITY) DO_QUANTITY, DOL.LINE_QUANTITY ORIGINAL_QUANTITY,
(CASE
 WHEN NVL(INVOICE_AMOUNT,0) = 0 THEN NULL
 WHEN (NVL(INVOICE_AMOUNT,0) - (NVL(AMOUNT_PAID,0)+NVL(DISCOUNT_AMOUNT_TAKEN,0))) = 0 THEN 'Fully Paid'
 WHEN NVL(AMOUNT_PAID,0) = 0 THEN 'Unpaid'
...
DECODE(:P_GROUP_BY, 'CUSTOMER', APS.VENDOR_NAME, CUST.CUSTOMER_NAME) PARTY_NAME,  INITCAP(MVH.MOV_ORDER_STATUS) MOV_ORDER_STATUS,  
DECODE (MVH.WAREHOUSE_ORG_ID, 101, MVH.WAREHOUSE_ORG_CODE, APPS.XXAKG_COM_PKG.GET_ORGANIZATION_NAME (MVH.WAREHOUSE_ORG_ID)) WAREHOUSE,  
DECODE(SOL.INVENTORY_ITEM_ID, 24408, (SOL.SHIPPED_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION),206570, (SOL.SHIPPED_QUANTITY*APPS.XXAKG_ONT_PKG.GET_BULK_CONVERSION), SOL.SHIPPED_QUANTITY) DO_QUANTITY, SOL.SHIPPED_QUANTITY ORIGINAL_QUANTITY,
'Uninvoiced'  BILL_STATUS, NULL PAY_DATE, MVH.TRANSPORT_MODE, MVH.FINAL_DESTINATION,
NVL (DOL.ACTUAL_RETAILER_SHIP_TO, CUST.LOCATION||DECODE(CUST.LOCATION_ADDRESS, NULL, NULL,', '||CUST.LOCATION_ADDRESS)) DELIVERY_LOCATION,
MVH.HIRE_RATE_AP HIRE_RATE,NULL VENDOR_SITE_CODE,
(select payable_rate from apps.xxakg_transport_hirerate 
